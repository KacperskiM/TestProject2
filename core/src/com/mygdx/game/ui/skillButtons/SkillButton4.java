...
public class SkillButton4 extends Button {
    ...
    public SkillButton4(GameplayScreen gameplayScreen) {
    ...
    private void reactOnPress() {
        if (gpScreen.getSelectedSource() instanceof Paladin) {
            ...
            gpScreen.getPaladin().getPaladinTooltip().drawTooltip(this.getStage().getBatch(), 1f, 4);
        } else if (gpScreen.getSelectedSource() instanceof Ranger) {
            this.getStage().addActor(gpScreen.getRanger().getRangerTooltip());
            this.getStage().getBatch().begin();
            gpScreen.getRanger().getRangerTooltip().drawTooltip(this.getStage().getBatch(), 1f, 4);
        } else if (gpScreen.getSelectedSource() instanceof Cleric) {
            this.getStage().addActor(gpScreen.getCleric().getClericTooltip());
            this.getStage().getBatch().begin();
            gpScreen.getCleric().getClericTooltip().drawTooltip(this.getStage().getBatch(), 1f, 4);
        }
        this.getStage().getBatch().end();
    }
    private void reactOnClick(){
        if(gpScreen.getSelectedTarget()== null || gpScreen.getSelectedSource().getCurrentMana()< gpScreen.getSelectedSource().getFourthSkillManaCost())
            return;
